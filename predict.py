...
def get_prediction(image_path: str, model, cfg: dict, threshold: float =0.5, verbose: int =1) -> (list, list, list):
    # load image
    ...
    INSTANCE_CATEGORY_NAMES = {COCO_CATEGORY["id"]: COCO_CATEGORY["name"] for COCO_CATEGORY in COCO_CATEGORIES}
...
def visualize_prediction(img_path: str, masks, boxes, pred_cls, rect_th:float =3, text_size:float =3, text_th: float =3, file_name:str ="inference_result.png"):
    # read image
...
def crop_inference_bbox(img_path, masks, boxes, file_name="cropped_inference_result.png"):
    # read image
    img = cv2.imread(img_path)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    # create output folder if not present
    ...
    if len(masks)>0:
        for i in range(len(masks)):
            cropped_img = img[int(boxes[i][0][1]):int(boxes[i][1][1]),int(boxes[i][0][0]):int(boxes[i][1][0]),:]
            plt.figure(figsize=(20,30))
...
def instance_segmentation_api(image_path: str, weight_path: str):
    ...
    cfg = model_dict["cfg"]
    # get prediction
    masks, boxes, pred_cls = get_prediction(image_path, model, cfg, threshold=0.75)
    # visualize result
    visualize_prediction(image_path, masks, boxes, pred_cls, rect_th=3, text_size=3, text_th=3)
...
if __name__ == '__main__':
    ...
    ap.add_argument("image_path", default = "test/test_files/CA/CA01_01.tif", help= "Path for input image.")
    ap.add_argument("weight_path", default = "artifacts/maskrcnn-best.pt", help= "Path for trained MaskRCNN model.")
